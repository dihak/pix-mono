...
Completed calls collapse after the configured Pix delay into a status row such as `✓ sudo apt install ripgrep · exit 0 · 18 lines`. Expanding the row restores the normal stdout/stderr preview. Approval and masked-password overlays are never collapsed, and passwords are never included in result metadata or render state.
